...
| `followup_llm_judge_enabled` | bool | `true` | 续聊窗口内同一发送者没有命中固定追问词时，是否让当前模型参与续聊打分。 |
| `followup_score_threshold` | int | `75` | 模型辅助续聊的最终触发分数，越高越保守。 |
| `followup_model_weight` | int | `30` | 模型判断像续聊时增加的分数；模型不会一票通过。 |
...
当小昭刚刚回复过某个发送者后，插件会记录这个“当前对话目标”。在 `followup_reply_window_sec` 秒内，如果同一群、同一机器人、同一发送者继续发出像追问、纠正、要求直接回答，或“评价一下/分析一下/讲讲/说说”这类明确让它接着处理某个主题的消息，即使没有再次提到 `mention_keywords`，插件也会接着回复。若 `followup_llm_judge_enabled=true`，没有命中固定追问词的自然接话会进入权重打分：本地规则先加减分，当前模型只提供有限加分，最终超过 `followup_score_threshold` 才继续回复。
